...
use Cartalyst\Sentry\Users\UsernameExistsException;
...
class User extends Model implements UserInterface {
	...
	/**
	 * Returns the name for the user's username.
	 *
	 * @return string
	 */
	public function getLoginUsername()
	{
		return static::$usernameAttribute;
	}
	...
	{
		return $this->{$this->getLoginUsername()};
	}
	...
	public function validate()
	{
		...
		$username=$this->{static::$usernameAttribute};
		
		
		$persistedUsername = $query->where('username', '=', $username)->first();
		

		if ($persistedUsername and $persistedUsername->getId() != $this->getId())
		{
		  
			throw new UserExistsException("A username already exists with login [$login], logins must be unique for users.");
		}
		
		
		$query1 = $this->newQuery();
		$persistedUser = $query1->where($this->getLoginName(), '=', $login)->first();
