...
<div class="picture post hide" >
  <form method="post" action="{{URL::to('post/picture')}}" enctype="multipart/form-data" >
    <input type="hidden" name="blog" value="{{$blog->name}}" />
    <input name="file[]" multiple="1" type="file" placeholder="Image" class="form-control" accept='image/*' >
...
<div class="quote post hide" >
  <form method="post" action="{{URL::to('post/quote')}}" >
    <input type="hidden" name="blog" value="{{$blog->name}}" />
    <textarea name="content" class="form-control" placeholder="Quote" ></textarea >
...
<div class="chat post hide" >
  <form method="post" action="{{URL::to('post/chat')}}" >
    ...
    <textarea name="content" class="form-control" placeholder="Tourist: Could you....." ></textarea >
...
<div class="feed clearfix" >
  <div class="avatar" >
    <a href="{{URL::route('publicblog', array('name' => $blog->name))}}" >
      <img src="{{asset(json_decode($blog->config)->picture)}}" class="dp" >
    </a >
...
<!--          </div >-->
        ...
        <div class="post_content" >
          ...
          <video width="90%" height="90%" controls src="{{URL::asset(json_decode($post->regularPost->content)->url)}}" type="video/mp4" id="player1"  controls="controls" preload="none" ></video >
...
@if($post->rebloggedPost->originalPost->regularPost['type'] == 'text')
    ...
    <div class="feed clearfix" >
      ...
      <div class="news panel" >
        <div class="post_content" >
          <div class="post_info" >
            <a href="{{URL::route('publicblog', array('username'=>$blog->author->username,'name' => $blog->name))}}" > {{json_decode($blog->config)->title}} </a > <i class="fas fa-retweet"></i> 
			<a href="{{URL::route('publicblog', array('username'=>$post->rebloggedPost->originalPost->blog->author->username,'name' => $post->rebloggedPost->originalPost->blog->name))}}"> {{json_decode($post->rebloggedPost->originalPost->blog->config)->title}} </a>
          </div >
           <div class="reblog_desc"> {{json_decode($post->rebloggedPost->content)->comment}}</div>
          <div class="reblog_content">
...
<aside class="sidebar" >
  <div class="btn-group" >
    <a type="button" class="switch dropdown-toggle" data-toggle="dropdown" >
      ...
      <span class="url" >{{URL::route('publicblog', array('name' => $blog->name))}} </span >
      <i class="fas fa-chevron-down" ></i >
    </a >
    <ul class="dropdown-menu" role="menu" >
      @foreach($blogs as $userblog)
      <li class="blogs" >
        <a href="{{URL::to('blog/'.$userblog->name)}}" class="clearfix" >
          <img src="{{asset(json_decode($userblog->config)->picture)}}" >
          <span class="name" > {{json_decode($userblog->config)->title}} </span >
          <span class="url" >{{URL::route('publicblog', array('name' => $userblog->name))}} </span >
  ...
  <ul class="nav nav-pills nav-stacked navigation" >
    ...
    <li ><a href="{{URL::route('customise', array('name' => $blog->name))}}" ><i class="far fa-eye" ></i >&nbsp; Customise</a ></li >
  </ul >
</aside >
</section >
</div >
