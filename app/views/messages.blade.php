  ...
  <section class="content clearfix" >
	...
	@if(Session::get('error'))
      ...
      <div class="feed clearfix" >
        <div class="avatar no-nip" >
          <a href="http://{{$sender->name}}.{{Config::get('database.domain')}}" >
    ...
    <aside class="sidebar" >
      ...
      <ul class="nav nav-pills nav-stacked navigation" >
        <li class="active" ><a href="{{URL::route('messages')}}" > All Messages </a ></li >
        @foreach($blogs as $blog)
        <li ><a href="{{URL::route('blogmessages', array('name' => $blog->name))}}" >{{json_decode($blog->config)->title}} <span >{{$blog->receivedMessages->count()}}</span ></a ></li >
        @endforeach
      </ul >
    </aside >
  </section >
    ...
    <div class="modal-dialog" >
        <div class="fanmail modal-content" >
            ...
            <form id="reply_form" class="compose" method="post" action="{{URL::to('message/send')}}" >
                ...
                <textarea id="reply_message" name="message" class="mono" placeholder="Message" required ></textarea >
